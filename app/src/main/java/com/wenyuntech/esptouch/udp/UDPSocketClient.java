package com.wenyuntech.esptouch.udp;
...
public class UDPSocketClient {
	...
	public void sendData(byte[][] data, String targetHostName, int targetPort,
			long interval) {
		Log.i(TAG, data + " targetHostName = " + targetHostName + " targetPort = " + targetPort + " interval = " + interval);
	...
	public void sendData(byte[][] data, int offset, int count,
		...
		for (int i = offset; !mIsStop && i < offset + count; i++) {
			...
			try {
                ...
                Log.i(TAG, "i = " + i + " data[i].length " + data[i].length);
