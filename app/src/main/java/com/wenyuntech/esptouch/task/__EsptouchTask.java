package com.wenyuntech.esptouch.task;
...
import com.wenyuntech.esptouch.EsptouchResult;
import com.wenyuntech.esptouch.IEsptouchListener;
import com.wenyuntech.esptouch.IEsptouchResult;
import com.wenyuntech.esptouch.prorocal.EsptouchGenerator;
import com.wenyuntech.esptouch.udp.UDPSocketClient;
import com.wenyuntech.esptouch.udp.UDPSocketServer;
import com.wenyuntech.esptouch.util.ByteUtil;
import com.wenyuntech.esptouch.util.EspNetUtil;
...
public class __EsptouchTask implements __IEsptouchTask {
		...
		mTask = new Thread() {
			public void run() {
				...
				while (mEsptouchResultList.size() < mParameter
					...
					Log.i(TAG, "while() " + mEsptouchResultList.size() + " " + mParameter.getExpectTaskResultCount());
					...
					if (receiveOneByte == expectOneByte) {
						if (__IEsptouchTask.DEBUG) {
							Log.i(TAG, "receive correct broadcast receiveOneByte = " + receiveBytes + " experctOneByte = " + expectOneByte);
						}
	...
	private boolean __execute(IEsptouchGenerator generator) {
		...
		int index = 0;
		while (!mIsInterrupt) {
            Log.i(TAG, currentTime - lastTime + " " + mParameter.getTimeoutTotalCodeMillisecond());
	...
	public List<IEsptouchResult> executeForResults(int expectTaskResultCount)
		...
		boolean isSuc = false;
        Log.i(TAG, "total repeatTime " + mParameter.getTotalRepeatTime());
